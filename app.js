...
function ruleBasedResponse(q) {
  ...
  scores.positive    = tokenScore(q2, [
    'good','great','nice','excellent','perfect','best','awesome','superb','amazing','brilliant',
    'fantastic','wonderful','well done','thank you','thanks','i like','like you','love it',
    'very good','so good','too good','so nice','very nice','helpful','impressive','appreciate',
    'fabulous','terrific','splendid','correct','right','you rock','good work','great work',
    'bravo','kudos','hats off','accha','acha','bahut acha','bahut badhiya','badhiya','badiya',
    'sahi','ekdum sahi','bilkul sahi','mast','zabardast','kamaal','kya baat','shabash',
    'wah','waah','shukriya','dhanyawad','bahut mast','lajawaab','lajawab','shandar',
    'behtareen','gazab','gajab','maza aaya','maja aaya','bahut helpful','pasand aaya',
    'pasand aayi','liked it','like kiya','number one','no 1','top class','first class',
    'satisfied','khush','happy','glad','bahut sahi ho','tum sahi ho','tum acha ho',
    'bhai sahi','yaar sahi','tu mast','tum best','you are best','you are amazing'
  ]);
  scores.negative    = tokenScore(q2, [
    'bad','wrong','incorrect','not good','not helpful','useless','worst','terrible','awful',
    'horrible','poor','disappointing','disappointed','unhappy','not satisfied','not happy',
    'dissatisfied','frustrated','annoying','pathetic','rubbish','garbage','waste','wasted',
    'not useful','not accurate','inaccurate','not correct','wrong answer','false','confused',
    'confusing','unclear','dont understand','did not help','failed','failure','mistake',
    'i hate','hate this','hate it','not impressed','totally wrong','fix this',
    'bura','bura laga','acha nahi laga','accha nahi laga','pasand nahi aaya','pasand nahi',
    'galat','bilkul galat','ekdum galat','sahi nahi','bekaar','bekar','faltu','faltoo',
    'kaam ka nahi','kisi kaam ka nahi','bakwas','bak bak','ganda','kharab','nafrat',
    'khafa','naraaz','naraz','dukhi','pareshan','nirash','nirasha','fail','nahi samjha',
    'ulta jawab','dhoka','dhokha','bhai galat','yaar galat','kya bata raha','samajh nahi aaya',
    'time waste','time barbaad','barbaad','kuch nahi bataya','help nahi kiya','fayda nahi',
    'disappointed hoon','khafa hoon','naraaz hoon','nahi acha','nahi acha laga'
  ]);
  ...
  // Negative feedback — takes priority
  var isNegFb = scores.negative >= 1 && scores.pump === 0 && scores.soil === 0 &&
    scores.rain === 0 && scores.irrigation === 0 && scores.sensor === 0 &&
    scores.crop === 0 && scores.water_qty === 0;
  if (isNegFb) best = 'negative';

  // Positive feedback
  var isPosFb = scores.positive >= 1 && scores.negative === 0 && scores.pump === 0 &&
    scores.soil === 0 && scores.rain === 0 && scores.irrigation === 0 &&
    scores.sensor === 0 && scores.crop === 0 && scores.water_qty === 0;
  if (isPosFb) best = 'positive';
  ...
  switch (best) {
    ...
    case 'negative': {
      ...
      aiMemory.corrections.push(qn);
      var base = '';
      if (/galat|wrong|incorrect|ulta|sahi nahi/.test(qn)) {
        base = '😔 Maafi! Mujhse galat jawab mila.\n\nKya aap dobara poochh sakte hain?\nMain seedha aur sahi jawab dunga!';
      } else if (/samajh nahi|confus|unclear|seedha nahi|kuch nahi bataya/.test(qn)) {
        base = '😔 Khed hai ki main clearly samjha nahi paya.\n\nDobara poochho — thoda simple mein:\n• Kaunsa topic — pump, mitti, paani, baarish?\n• Kya exactly jaanna chahte ho?';
      } else if (/bekaar|bekar|faltu|useless|waste|fayda nahi/.test(qn)) {
        base = '😔 Hume khed hai ki hum aapki sahayata nahi kar paye.\n\nMain seedha sawaal pe behtar jawab deta hoon. Dobara poochho!\n\n💡 Example:\n• "Pump kyun band hai"\n• "Mitti kitni dry hai"\n• "Aaj kitna paani dena hai"';
      } else if (/hate|nafrat|ganda|kharab|bura laga|pasand nahi/.test(qn)) {
        base = '😔 Hume khed hai ki aapko achha nahi laga.\n\nAapka feedback important hai — batao kya problem thi, main sudharne ki koshish karoonga.';
      } else if (/disappointed|nirash|naraaz|khafa|dukhi|unhappy|nahi acha/.test(qn)) {
        base = '😔 Hume sachchi khed hai ki aap santusht nahi hue.\n\nKya aap batayenge kahan galat hua? Main poori koshish karoonga!';
      } else {
        base = '😔 Hume khed hai ki hum aapki sahayata nahi kar paye.\n\nDobara poochho — is baar behtar jawab dunga! 🙏';
      }
      var sug = '';
      if (lastT_n && lastT_n !== 'negative' && lastT_n !== 'default') {
        sug = '\n\n💡 Pichla topic: ' + lastT_n + ' — wahi dobara poochhen?';
      }
      var fs = d_n ? '\n\n📊 Farm: CSMI ' + fv_n(d_n.csmi).toFixed(1) + '% | Pump ' + (d_n.pump ? '🟢 ON' : '🔴 OFF') : '';
      return base + sug + fs;
    }
    ...
    case 'positive': {
      ...
      var lastT_p = aiMemory.lastTopic;
      var qp = q.toLowerCase();
      var resp = '';
      if (/bhai|yaar|dost|friend/.test(qp)) {
      ...
      if (lastT_p === 'pump')      resp += '\n\n💡 Pump ke baare mein kuch aur?';
      else if (lastT_p === 'soil') resp += '\n\n💡 Mitti ke baare mein kuch aur?';
      else if (lastT_p === 'rain') resp += '\n\n💡 Baarish ke baare mein kuch aur?';
      else if (lastT_p === 'irrigation' || lastT_p === 'water_qty') resp += '\n\n💡 Sinchai ke baare mein kuch aur?';
    ...
    // ══ DEFAULT ═══════════════════════════════════════════════
    default: {
      // Try to use last topic context
      var contextHint = '';
      if (aiMemory.lastTopic && aiMemory.lastTopic !== 'default') {
        contextHint = '\n\n(Pehle humne "' + aiMemory.lastTopic + '" ke baare mein baat ki thi.)';
      }
      var farmSnap = d ?
        'Abhi: CSMI ' + fv(d.csmi).toFixed(1) + '% | Pump ' + (d.pump ? 'ON' : 'OFF') + ' | Rain ' + (d && fv(d.rainProb) > 0 ? fv(d.rainProb).toFixed(0) : (owm ? getRainInfo().prob.toFixed(0) : '0')) + '%' :
        'Hardware connected nahi.';
      return '🤔 Yeh sawaal naya laga: "' + q.substring(0, 50) + '"\n\n' +
        farmSnap + contextHint + '\n\n' +
        'Kuch aisa puchho:\n' +
        '• "Pump kyun band hai"\n' +
        '• "Mitti kaisi hai abhi"\n' +
        '• "Aaj kitna paani dena hai"\n' +
        '• "Baarish aayegi kya"\n' +
        '• "Sab theek hai kya"\n\n' +
        '"help" type karo full list ke liye!';
    }
  }
}
